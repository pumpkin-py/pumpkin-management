...
async def create_channel(category: discord.CategoryChannel, bitrate: int = None):
...
async def remove_channel(channel: discord.VoiceChannel):
...
class Voice(commands.Cog):
    ...
    async def _check_and_sync(self, guild: discord.Guild):
        ...
        category = _get_category(guild, settings.category_id)
        if not category:
    ...
    async def _sync(self, setting: VoiceSettings):
        ...
        category = _get_category(guild, setting.category_id)
        if not category:
            return
        if setting.high_res_bitrate:
            ...
            empty_channels = list(
                filter(
                    ...
                    and x.name.startswith(HIGH_RES_CHANNEL_PREFIX)
                    and len(list(x.members)) == 0
                    else False,
                    category.channels,
                )
            )
            ...
            if len(empty_channels) - len(abandoned_channels) < 1:
                await create_channel(category, setting.high_res_bitrate)
            for abandoned in abandoned_channels:
                await remove_channel(abandoned)
        ...
        empty_channels = list(
            filter(
                lambda x: True
                if isinstance(x, discord.VoiceChannel)
                and not x.name.startswith(HIGH_RES_CHANNEL_PREFIX)
                and len(list(x.members)) == 0
                else False,
                category.channels,
            )
    ...
    async def on_voice_state_update(self, member, beforeState, afterState):
        ...
        category = _get_category(member.guild, setting.category_id)
        ...
        if after:
            ...
            if after.category.id == setting.category_id:
                if not after.last_message:
                    LockedChannels.unlock(member.guild, after)
                    await _send_welcome_message(after)
        ...
        if before:
            if after.category.id == setting.category_id:
                ...
                if len(list(before.members)) < 1:
                    ...
                    await remove_channel(before)

    @commands.guild_only()
    @check.acl2(check.ACLevel.MEMBER)
    @commands.cooldown(1, 10, commands.BucketType.channel)
    @commands.check(check_is_in_voice)
    @commands.check(check_active_settings)
    ...
    async def lock(self, ctx):
        ...
        if LockedChannels.is_locked(ctx.channel):
            ...
            return
        LockedChannels.lock(ctx.guild, ctx.channel)
        settings = VoiceSettings.get(ctx.guild)
        category = _get_category(ctx.guild, settings.category_id)
    ...
    async def unlock(self, ctx):
        ...
        if not LockedChannels.is_locked(ctx.channel):
            ...
            return
        LockedChannels.unlock(ctx.guild, ctx.channel)
        settings = VoiceSettings.get(ctx.guild)
        category = _get_category(ctx.guild, settings.category_id)
        for role in category.overwrites:
            if not isinstance(role, discord.Role):
    ...
    @voice.command(
        name="category",
        aliases=["set_category", "category_set", "set-category", "category-set"],
    )
    ...
    @voice.command(
        name="bitrate",
        aliases=["set_bitrate", "bitrate_set", "set-bitrate", "bitrate-set"],
    )
